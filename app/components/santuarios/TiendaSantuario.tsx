...
import { useWineFilter, Currency } from '@/app/hooks/useWineFilter'; // <-- IMPORTAMOS LA NUEVA LEY
...
const FilterTab: React.FC<{label: string; isActive: boolean; onClick: () => void; children?: React.ReactNode}> = ({ label, isActive, onClick, children }) => (
  <div className="flex items-center">
    ...
    {/* Renderizamos contenido adicional DENTRO de la pestaña */}
    {children}
  </div>
);
...
const TiendaSantuario: React.FC = () => {
  ...
  // 1. INVOCAMOS AL NUEVO CEREBRO BIMONETARIO
  const {
  ...
  return (
    <section className="flex w-full h-screen bg-black text-white pt-20 overflow-hidden">
      <div className="w-1/2 h-full flex-shrink-0">
        <PergaminoMaestro>
            ...
            <div className="flex border-b border-bodega-gold/10 mb-4 shrink-0">
              {filterTabs.map(tab => (
                <FilterTab key={tab} label={tab} isActive={activeTab === tab} onClick={() => setActiveTab(tab)}>
                  {/* 2. EL INTERRUPTOR DE REALIDAD SE DESPLIEGA EN SU PUESTO */}
                  {tab === 'Precio' && (
                    <div className="flex items-center space-x-1 text-xs ml-2 bg-black/20 p-0.5 rounded-md border border-bodega-gold/10">
            ...
            <div className="mb-4 shrink-0 min-h-[90px] flex flex-col justify-center">
              <AnimatePresence mode="wait">
                <motion.div key={activeTab} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} transition={{ duration: 0.2 }}>
                  ...
                  {/* 3. El RangeSlider ahora es consciente de la divisa */}
            ...
            <motion.div className="grid grid-cols-4 gap-4 overflow-y-auto flex-1 min-h-0 [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none] p-1">
              {/* 4. Las tropas reciben la orden de la divisa */}
      ...
      <div className="w-1/2 h-full relative">
        ...
        <AnimatePresence>
          {selectedVino && <motion.div className="absolute inset-0 z-20 flex items-center justify-center p-8 md:p-12" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}><div className="absolute inset-0 bg-black/50 backdrop-blur-sm" onClick={() => setSelectedVino(null)} /><motion.div className="relative w-full h-full max-w-md max-h-[90vh] bg-bodega-dark/60 backdrop-blur-xl rounded-2xl border border-bodega-gold/30 shadow-2xl shadow-black/50 overflow-hidden">
            {/* 5. El Altar también recibe la orden de la divisa */}
            <WineDetail vino={selectedVino} onClose={() => setSelectedVino(null)} currency={currency} />
