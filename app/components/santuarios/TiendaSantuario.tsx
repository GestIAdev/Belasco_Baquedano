...
const filterTabs = ['Tipo', 'Varietal', 'Maridaje', 'Precio', 'Añada']; // <-- PESTAÑAS SEPARADAS
...
const FilterTab: React.FC<{label: string; isActive: boolean; onClick: () => void}> = ({ label, isActive, onClick }) => (
  <button onClick={onClick} className="relative px-4 py-3 text-lg transition-colors duration-200 focus:outline-none">
    ...
    {isActive && (
      <motion.div className="absolute bottom-0 left-0 right-0 h-0.5 bg-bodega-gold" layoutId="active-filter-underline" />
    )}
  </button>
);
...
const TiendaSantuario: React.FC = () => {
  ...
  const {
    filteredVinos, selectedTipos, selectedVarietals, selectedMaridajes, priceRange, vintageRange,
    priceBounds, vintageBounds,
    handleToggle, setPriceRange, setVintageRange,
    ...
    <section className="flex w-full h-screen bg-black text-white pt-20 overflow-hidden">
      <div className="w-1/2 h-full flex-shrink-0">
        <PergaminoMaestro>
            ...
            <div className="flex border-b border-bodega-gold/10 mb-4 shrink-0">
              {filterTabs.map(tab => <FilterTab key={tab} label={tab} isActive={activeTab === tab} onClick={() => setActiveTab(tab)} />)}
            </div>
            ...
            <div className="mb-4 shrink-0 min-h-[90px] flex flex-col justify-center">
              <AnimatePresence mode="wait">
                <motion.div key={activeTab} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} transition={{ duration: 0.2 }}>
                  ...
                  {activeTab === 'Precio' && <div className="pt-6 px-2"><RangeSlider prefix="$" min={priceBounds.min} max={priceBounds.max} initialMin={priceRange.min} initialMax={priceRange.max} onChange={setPriceRange} /></div>}
            ...
            <motion.div className="grid grid-cols-4 gap-4 overflow-y-auto flex-1 min-h-0 [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none] p-1">
              {filteredVinos.map(vino => <WineCard key={vino.id} vino={vino} onSelect={setSelectedVino} />)}
            </motion.div>
        </PergaminoMaestro>
      </div>
      ...
      <div className="w-1/2 h-full relative">
        ...
        <div className="relative z-10 w-full h-full flex flex-col items-center justify-center p-8 text-center text-white/80"> 
          <AnimatePresence>
            {!selectedVino && <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}><h2 className="text-4xl font-serif text-bodega-gold/80">Nuestra Colección</h2><p className="mt-4 text-bodega-stone">Seleccione un vino para descubrir su historia.</p></motion.div>}
          </AnimatePresence>
        </div>
        <AnimatePresence>
          {selectedVino && <motion.div className="absolute inset-0 z-20 flex items-center justify-center p-8 md-p-12" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}><div className="absolute inset-0 bg-black/50 backdrop-blur-sm" onClick={() => setSelectedVino(null)} /><motion.div className="relative w-full h-full max-w-md max-h-[90vh] bg-bodega-dark/60 backdrop-blur-xl rounded-2xl border border-bodega-gold/30 shadow-2xl shadow-black/50 overflow-hidden"><WineDetail vino={selectedVino} onClose={() => setSelectedVino(null)} /></motion.div></motion.div>}
        </AnimatePresence>
      </div>
    </section>
  );
