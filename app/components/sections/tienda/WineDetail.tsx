...
const WineDetail: React.FC<{vino: Vino, onClose: () => void}> = ({ vino, onClose }) => {
    const notasList = vino.notas_cata.split(',').map(n => n.trim());

    // Función para formatear precios argentinos, por si acaso ;)
    ...
    return (
        <div className="w-full h-full flex flex-col text-white relative font-sans">
            <button onClick={onClose} className="absolute top-4 right-4 text-bodega-gold/50 hover:text-bodega-gold transition-colors z-30">
            ...
            <div className="w-full h-1/2 flex items-center justify-center pt-8">
                ...
                >
                    ...
                    <motion.img 
                        ...
                        onError={(e) => e.currentTarget.src = 'https://placehold.co/400x600/1a1a1a/a88b57?text=Vino'} 
                    />
                </motion.div>
            </div>
            ...
            <div className="w-full flex-1 flex flex-col p-4 overflow-hidden min-h-0">
                ...
                <div className="flex justify-between gap-x-4 text-xs text-left max-w-lg mx-auto w-full border-t border-bodega-gold/10 pt-4">
                    <div className="flex-1">
                        ...
                        <ul className="list-disc list-inside text-bodega-stone/80 font-light space-y-1">{notasList.map((nota, i) => <li key={i}>{nota}</li>)}</ul>
                    </div>
                    <div className="flex-1">
                        <h4 className="font-bold text-bodega-gold/90 mb-2 uppercase tracking-wider">Maridaje</h4>
                        <ul className="list-disc list-inside text-bodega-stone/80 font-light space-y-1">{vino.maridaje.map((item, i) => <li key={i}>{item}</li>)}</ul>
                    </div>
                    <div className="flex-1">
                        <h4 className="font-bold text-bodega-gold/90 mb-2 uppercase tracking-wider">Aromas</h4>
                        <div className="flex flex-wrap gap-1.5">
                            {vino.aromas.map((aromaId, i) => <MicroAroma key={i} aromaId={aromaId} />)}
                        </div>
                    </div>
                </div>
                ...
                <div className="mt-auto pt-6">
                    <div className="flex items-center justify-between gap-x-6 border-t border-bodega-gold/10 pt-4">
                         ...
                         <div className="flex-1 text-left">
                            <p className="text-2xl font-bold text-white/80 whitespace-nowrap">{formatPrice(vino.precio)}</p>
                         </div>
                        {/* Botón de Conquista */}
                        <motion.button 
                            className="bg-bodega-gold text-bodega-dark font-bold py-3 px-6 rounded-full flex items-center justify-center gap-x-2 hover:bg-white transition-colors whitespace-nowrap"
                            whileTap={{ scale: 0.95 }}
                        >
